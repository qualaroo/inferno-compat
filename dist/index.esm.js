import { render, _CI, _MCCC, _HI, _MFCC, _ME, _MP, _MR, _MT, _M, Component, EMPTY_OBJ, Fragment, __render, createComponentVNode, createFragment, createPortal, createRef, createRenderer, createTextVNode, createVNode, directClone, findDOMfromVNode, forwardRef, getFlagsForElementVnode, linkEvent, normalizeProps, options, rerender } from 'inferno';
export { Component, EMPTY_OBJ, Fragment, _CI, _HI, _M, _MCCC, _ME, _MFCC, _MP, _MR, _MT, __render, createComponentVNode, createFragment, createPortal, createRef, createRenderer, createTextVNode, createVNode, directClone, findDOMfromVNode, forwardRef, getFlagsForElementVnode, linkEvent, normalizeProps, options, rerender } from 'inferno';
...
function hydrateComponent(vNode, parentDOM, dom, context, isSVG, isClass, lifecycle, doc) {
    var type = vNode.type;
    var ref = vNode.ref;
    ...
    if (isClass) {
        ...
        currentNode = hydrateVNode(input, parentDOM, dom, instance.$CX, isSVG, lifecycle, doc);
        _MCCC(ref, instance, lifecycle);
    }
    else {
        var input$1 = _HI(type(props, context));
        currentNode = hydrateVNode(input$1, parentDOM, dom, context, isSVG, lifecycle, doc);
        vNode.children = input$1;
        _MFCC(props, ref, vNode, lifecycle);
    }
    return currentNode;
}
function hydrateChildren(parentVNode, parentNode, currentNode, context, isSVG, lifecycle, doc) {
    ...
    if (childFlags !== 1 /* HasInvalidChildren */) {
        if (childFlags === 2 /* HasVNodeChildren */) {
            if (isNull(currentNode)) {
                _M(children, parentNode, context, isSVG, null, lifecycle, doc);
            }
            else {
                currentNode = hydrateVNode(children, parentNode, currentNode, context, isSVG, lifecycle, doc);
                currentNode = currentNode ? currentNode.nextSibling : null;
            }
        }
        else if (childFlags === 16 /* HasTextChildren */) {
            if (isNull(currentNode)) {
                parentNode.appendChild(doc.createTextNode(children));
            }
            ...
            for (var i = 0, len = children.length; i < len; ++i) {
                ...
                if (isNull(currentNode) || (prevVNodeIsTextNode && (child.flags & 16 /* Text */) > 0)) {
                    _M(child, parentNode, context, isSVG, currentNode, lifecycle, doc);
                }
                else {
                    currentNode = hydrateVNode(child, parentNode, currentNode, context, isSVG, lifecycle, doc);
                    currentNode = currentNode ? currentNode.nextSibling : null;
                }
...
function hydrateElement(vNode, parentDOM, dom, context, isSVG, lifecycle, doc) {
    var props = vNode.props;
    ...
    if (dom.nodeType !== 1 || dom.tagName.toLowerCase() !== vNode.type) {
        _ME(vNode, null, context, isSVG, null, lifecycle, doc);
        parentDOM.replaceChild(vNode.dom, dom);
    }
    else {
        vNode.dom = dom;
        hydrateChildren(vNode, dom, dom.firstChild, context, isSVG, lifecycle, doc);
        if (!isNull(props)) {
            _MP(vNode, flags, props, dom, isSVG, doc);
        }
...
function hydrateText(vNode, parentDOM, dom, doc) {
    if (dom.nodeType !== 3) {
...
function hydrateFragment(vNode, parentDOM, dom, context, isSVG, lifecycle, doc) {
    var children = vNode.children;
    if (vNode.childFlags === 2 /* HasVNodeChildren */) {
        hydrateText(children, parentDOM, dom, doc);
        return children.dom;
    }
    hydrateChildren(vNode, parentDOM, dom, context, isSVG, lifecycle, doc);
    return findLastDOMFromVNode(children[children.length - 1]);
}
function hydrateVNode(vNode, parentDOM, currentDom, context, isSVG, lifecycle, doc) {
    var flags = (vNode.flags |= 16384 /* InUse */);
    if (flags & 14 /* Component */) {
        return hydrateComponent(vNode, parentDOM, currentDom, context, isSVG, (flags & 4 /* ComponentClass */) > 0, lifecycle, doc);
    }
    if (flags & 481 /* Element */) {
        return hydrateElement(vNode, parentDOM, currentDom, context, isSVG, lifecycle, doc);
    }
    if (flags & 16 /* Text */) {
        return hydrateText(vNode, parentDOM, currentDom, doc);
    }
    if (flags & 512 /* Void */) {
        return (vNode.dom = currentDom);
    }
    if (flags & 8192 /* Fragment */) {
        return hydrateFragment(vNode, parentDOM, currentDom, context, isSVG, lifecycle, doc);
    }
    throwError();
    return null;
}
function hydrate(input, parentDOM, callback) {
    ...
    if (isNull(dom)) {
        ...
        var lifecycle = [];
        if (!isInvalid(input)) {
            var doc = parentDOM.ownerDocument || document;
            dom = hydrateVNode(input, parentDOM, dom, {}, false, lifecycle, doc);
        }
        // clear any other DOM nodes, there should be only a single entry for the root
...
var PureComponent = /*@__PURE__*/(function (Component$$1) {
...
var WrapperComponent = /*@__PURE__*/(function (Component$$1) {
